...
if(isset($_POST['add_product'])){
   ...
   $select_product_name = mysqli_query($conn, "SELECT nama FROM `produk` WHERE nama = '$name'") or die('query failed');

   if(mysqli_num_rows($select_product_name) > 0){
      $message[] = 'Nama produk sudah ditambahkan!';
   }else{
      $add_product_query = mysqli_query($conn, "INSERT INTO `produk`(nama, harga, image) VALUES('$name', '$price', '$image')") or die('query failed');
...
if(isset($_GET['delete'])){
   ...
   $delete_image_query = mysqli_query($conn, "SELECT image FROM `produk` WHERE id = '$delete_id'") or die('query failed');
   $fetch_delete_image = mysqli_fetch_assoc($delete_image_query);
   unlink('img/'.$fetch_delete_image['image']);
   mysqli_query($conn, "DELETE FROM `produk` WHERE id = '$delete_id'") or die('query failed');
...
if(isset($_POST['update_product'])){
   ...
   mysqli_query($conn, "UPDATE `produk` SET nama = '$update_name', harga = '$update_price' WHERE id = '$update_p_id'") or die('query failed');
   ...
   if(!empty($update_image)){
      if($update_image_size > 2000000){
         ...
         mysqli_query($conn, "UPDATE `produk` SET image = '$update_image' WHERE id = '$update_p_id'") or die('query failed');
...
<section class="add-products">
   ...
   <form action="" method="post" enctype="multipart/form-data">
      ...
      <input type="file" name="image" accept="img/jpg, img/jpeg, img/png" class="box" required>
...
<section class="edit-product-form">
   ...
   <?php
      if(isset($_GET['update'])){
         ...
         $update_query = mysqli_query($conn, "SELECT * FROM `produk` WHERE id = '$update_id'") or die('query failed');
   ...
   <form action="" method="post" enctype="multipart/form-data">
      ...
      <input type="text" name="update_name" value="<?php echo $fetch_update['nama']; ?>" class="box" required placeholder="masukkan nama">
